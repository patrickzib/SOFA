...
#pragma omp parallel for num_threads(maxquerythread) reduction(min : bsf_distance)
    for (unsigned long j = 0; j < index->sax_cache_size; j++) {
        sax_type *sax = &index->sax_cache[j * index->settings->n_segments];
        if (minidist_paa_to_isax_rawa_SIMD(paa, sax, index->settings->max_sax_cardinalities, index->settings) <= bsf_distance) {
            ts_buffer = &rawfile[j * index->settings->timeseries_size];
...
/* This is intentionally enabled only by --profile-query-phases.  Timing each
 * primitive would distort a normal benchmark, but it gives a direct split of
...
void insert_tree_node_m(float *paa, isax_node *node, isax_index *index, float bsf, pqueue_t *pq,
    ...
    if (distance <= bsf) {
        if (node->is_leaf) {
            ...
            mindist_result->node = node;
            mindist_result->distance = distance;
            mindist_result->record_position = QUERY_RESULT_NO_POSITION;
