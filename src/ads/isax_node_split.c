...
int informed_split_decision(isax_node_split_data *split_data, isax_index_settings *settings,
        isax_node_record *records_buffer, int records_buffer_size) {
...
void split_node(isax_index *index, isax_node *node) {
    ...
    if (node->buffer->full_buffer_size > 0) {
        for (i = node->buffer->full_buffer_size - 1; i >= 0; i--) {
            ...
            split_buffer[split_buffer_index].insertion_mode = FULL_OR_NO_TMP;
            split_buffer_index++;
        }
    }
    ...
    if (node->buffer->partial_buffer_size > 0) {
        for (i = node->buffer->partial_buffer_size - 1; i >= 0; i--) {
            ...
            split_buffer[split_buffer_index].insertion_mode = PARTIAL_OR_NO_TMP;
            split_buffer_index++;
        }
    }
    ...
    if (node->buffer->tmp_full_buffer_size > 0) {
        for (i = node->buffer->tmp_full_buffer_size - 1; i >= 0; i--) {
            ...
            split_buffer[split_buffer_index].insertion_mode = TMP_OR_FULL;
            split_buffer_index++;
        }
    }
    ...
    if (node->buffer->tmp_partial_buffer_size > 0) {
        for (i = node->buffer->tmp_partial_buffer_size - 1; i >= 0; i--) {
            ...
            split_buffer[split_buffer_index].insertion_mode = PARTIAL_OR_TMP;
            split_buffer_index++;
        }
    }
    ...
    if (node->filename != NULL) {
        ...
        if (full_file != NULL) {
            ...
            while (!feof(full_file)) {
                ...
                if (!fread(split_buffer[split_buffer_index].position, sizeof(file_position_type), 1, full_file)) {
...
void split_node_m(isax_index *index, isax_node *node, pthread_mutex_t *lock_index) {
    // *******************************************************
    // CREATE TWO NEW NODES AND SET OLD ONE AS AN INTERMEDIATE
    // *******************************************************
    int i;
    ...
    if (node->filename != NULL) {
        ...
        if (full_file != NULL) {
            ...
            while (!feof(full_file)) {
                ...
                // If it can't read continue.
                if (!fread(split_buffer[split_buffer_index].position,
