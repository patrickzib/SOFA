...
query_result exact_search_serial_ParIS_nb_inmemory(ts_type *ts, ts_type *paa,
    ...
    if (approximate_result.distance == FLT_MAX || min_checked_leaves > 1) {
        approximate_result = refine_answer_inmemory(ts, paa, index,
                approximate_result, minimum_distance, min_checked_leaves);
    }
...
void exact_search_serial_ParIS_nb_batch_inmemory(ts_type *ts, ts_type *paa,
        isax_index *index, float minimum_distance, int min_checked_leaves, int batch_number) {
    RESET_BYTES_ACCESSED
...
#else
            if (minidist_paa_to_isax_raw(&(paa[i * index->settings->paa_segments]), sax,
                    index->settings->max_sax_cardinalities,
                    index->settings->sax_bit_cardinality,
                    index->settings->sax_alphabet_cardinality,
                    index->settings->paa_segments, MINVAL, MAXVAL,
...
void *ParIS_nb_worker_inmemory(void *worker_data) {
    ...
    for (i = start_number; i < stop_number; i++) {
        ...
        if (mindist <= ((ParIS_LDCW_data *) worker_data)->bsfdistance) {
            ts_buffer = &((ParIS_LDCW_data *) worker_data)->rawfile[i * index->settings->ts_byte_size /
                                                                    sizeof(ts_type)];
            COUNT_INPUT_TIME_END
...
query_result exact_search_parads_inmemory(ts_type *ts, ts_type *paa,
        isax_index *index, float minimum_distance, int min_checked_leaves) {
...
query_result exact_search_serial_ParIS_inmemory(ts_type *ts, ts_type *paa,
        isax_index *index, float minimum_distance, int min_checked_leaves) {
    RESET_BYTES_ACCESSED
    pthread_t threadid[maxquerythread];
    COUNT_INPUT_TIME_START
...
pqueue_bsf exact_topk_serial_ParIS_inmemory(ts_type *ts, ts_type *paa,
        isax_index *index, float minimum_distance, int min_checked_leaves, int k) {
    RESET_BYTES_ACCESSED
...
query_result exact_search_serial_ParIS2_inmemory(ts_type *ts, ts_type *paa,
        isax_index *index, float minimum_distance, int min_checked_leaves) {
    RESET_BYTES_ACCESSED
...
query_result
exact_search_serial_ParIS_openmp_inmemory(ts_type *ts, ts_type *paa,
        isax_index *index, float minimum_distance, int min_checked_leaves) {
    RESET_BYTES_ACCESSED
...
#else
        if (minidist_paa_to_isax_raw(paa, sax, index->settings->max_sax_cardinalities,
                index->settings->sax_bit_cardinality,
                index->settings->sax_alphabet_cardinality,
                index->settings->paa_segments, MINVAL, MAXVAL,
                index->settings->mindist_sqrt) <= bsf_distance) {
...
query_result exact_search_serial_ParGISG_openmp_inmemory(ts_type *ts, ts_type *paa,
        isax_index *index, float minimum_distance, int min_checked_leaves) {
    RESET_BYTES_ACCESSED
    pthread_t threadid[maxquerythread];
    COUNT_INPUT_TIME_START
...
query_result exact_search_serial_ParGIS_openmp_inmemory(ts_type *ts, ts_type *paa,
        isax_index *index, float minimum_distance, int min_checked_leaves) {
    RESET_BYTES_ACCESSED
    pthread_t threadid[maxquerythread];
    COUNT_INPUT_TIME_START
...
query_result exact_search_ParISnew_inmemory(ts_type *ts, ts_type *paa,
        isax_index *index, node_list *nodelist, float minimum_distance, int min_checked_leaves) {
    query_result approximate_result = approximate_search_inmemory_pRecBuf(ts, paa, index);
    query_result bsf_result = approximate_result;
    int tight_bound = index->settings->tight_bound;
    int aggressive_check = index->settings->aggressive_check;
    ...
    if (approximate_result.distance == FLT_MAX || min_checked_leaves > 1) {
        approximate_result = refine_answer_inmemory_m(ts, paa, index,
                approximate_result, minimum_distance, min_checked_leaves);
    }
...
query_result exact_search_ParISnew_inmemory_workstealing(ts_type *ts, ts_type *paa,
        isax_index *index, node_list *nodelist, float minimum_distance, int min_checked_leaves) {
    query_result approximate_result = approximate_search_inmemory_pRecBuf(ts, paa, index);
    query_result bsf_result = approximate_result;
    int tight_bound = index->settings->tight_bound;
    ...
    pqueue_t **allpq = (pqueue_t**)malloc(sizeof(pqueue_t *) * maxquerythread);
    pqueue_t *pq = pqueue_init(index->settings->root_nodes_size,
            cmp_pri, get_pri, set_pri, get_pos, set_pos);
    ...
    for (int i = 0; i < maxquerythread; i++) {
        ...
        queuelabel[i] = 1;
        localstk[i].val = (isax_node**)malloc(sizeof(isax_node *) * 100);
        localstk[i].top = 0;
        localstk[i].bottom = 0;
...
query_result exact_search_MESSI(ts_type *ts, ts_type *paa,
        isax_index *index, node_list *nodelist, float minimum_distance, int min_checked_leaves) {
    query_result approximate_result = approximate_search_inmemory_pRecBuf(ts, paa, index);
    query_result bsf_result = approximate_result;
    int tight_bound = index->settings->tight_bound;
...
query_result exact_search_ParISnew_inmemory_hybrid_workstealing(ts_type *ts, ts_type *paa,
        isax_index *index, node_list *nodelist, float minimum_distance, int min_checked_leaves) {
    query_result approximate_result = approximate_search_inmemory_pRecBuf(ts, paa, index);
    query_result bsf_result = approximate_result;
    int tight_bound = index->settings->tight_bound;
    ...
    for (int i = 0; i < maxquerythread; i++) {
        pthread_create(&(threadid[i]), NULL, exact_search_worker_inmemory_hybridpqueue_workstealing,
                (void *) &(workerdata[i]));
    }
...
void *exact_search_worker_inmemory(void *rfdata) {
    ...
    while (1) {
        ...
        insert_tree_node_m(paa, current_root_node, index, bsfdisntance, pq,
                ((refind_answer_fonction_data *) rfdata)->lock_queue);
    }
...
void *exact_search_worker_inmemory_workstealing(void *rfdata) {
    ...
    while (1) {
        ...
        insert_tree_node_m_workstealing(paa, current_root_node, index, bsfdisntance, pq,
                ((MESSI_workerdata *) rfdata)->lock_queue, localstk);
    }
    ...
    while (1) {
        ...
        for (int i = 0; i < maxquerythread; i++) {
            if (((MESSI_workerdata *) rfdata)->allstk[(i + offset) % maxquerythread].bottom != 0) {
                ...
                if (newnode != NULL) {
                    insert_tree_node_m_workstealing(paa, newnode, index, bsfdisntance, pq,
                            ((MESSI_workerdata *) rfdata)->lock_queue, localstk);
                }
            }
        }
...
void *exact_search_worker_inmemory_hybridpqueue(void *rfdata) {
    ...
    while (1) {
        ...
        current_root_node = ((MESSI_workerdata *) rfdata)->nodelist[current_root_node_number];
        insert_tree_node_m_hybridpqueue(paa, current_root_node, index, bsfdistance,
            ...
            if (n->node->is_leaf) {
                ...
                if (index->settings->function_type == 4) {
                    ...
                    if (index->settings->SIMD_flag) {
                        distance = calculate_node_distance2_inmemory_SFA_SIMD(index, n->node, ts, paa, bsfdistance);
                    } else {
#endif
                        distance = calculate_node_distance2_inmemory_SFA(index, n->node, ts, paa, bsfdistance);
#if defined(__x86_64__)
                    }
#endif
                } else {
                    distance = calculate_node_distance2_inmemory(index, n->node, ts, paa, bsfdistance);
                }

                if (distance < bsfdistance) {
                    pthread_rwlock_wrlock(((MESSI_workerdata *) rfdata)->lock_bsf);
...
void *exact_search_worker_inmemory_hybridpqueue_workstealing(void *rfdata) {
    ...
    while (1) {
        ...
        current_root_node = ((MESSI_workerdata *) rfdata)->nodelist[current_root_node_number];
        insert_tree_node_m_hybridpqueue_workstealing(paa, current_root_node, index, bsfdisntance,
                ((MESSI_workerdata *) rfdata)->allpq,
                ((MESSI_workerdata *) rfdata)->alllock, localstk, &tnumber);
    }
    ...
    while (1) {
        ...
        for (int i = 0; i < maxquerythread; i++) {
            if (((MESSI_workerdata *) rfdata)->allstk[(i + offset) % maxquerythread].bottom != 0) {
                ...
                if (newnode != NULL) {
                    insert_tree_node_m_hybridpqueue_workstealing(paa, newnode, index, bsfdisntance,
                            ((MESSI_workerdata *) rfdata)->allpq,
                            ((MESSI_workerdata *) rfdata)->alllock, localstk,
                            &tnumber);
                }
            }
        }
...
void insert_tree_node_m(float *paa, isax_node *node,
        isax_index *index, float bsf, pqueue_t *pq, pthread_mutex_t *lock_queue) {
    float distance = minidist_paa_to_isax(paa, node->isax_values,
            node->isax_cardinalities,
            index->settings->sax_bit_cardinality,
            index->settings->sax_alphabet_cardinality,
...
void insert_tree_node_mgpu(float *paa, isax_node *node,
        isax_index *index, float bsf, pqueue_bsf *pq, pthread_mutex_t *lock_queue) {
    float distance = minidist_paa_to_isax(paa, node->isax_values,
            node->isax_cardinalities,
            index->settings->sax_bit_cardinality,
            index->settings->sax_alphabet_cardinality,
...
void insert_tree_node_m_workstealing(float *paa, isax_node *node,
        isax_index *index, float bsf, pqueue_t *pq, pthread_mutex_t *lock_queue, localStack *workstack) {
    float distance = minidist_paa_to_isax(paa, node->isax_values,
            node->isax_cardinalities,
            index->settings->sax_bit_cardinality,
            index->settings->sax_alphabet_cardinality,
...
void insert_tree_node_m_hybridpqueue_workstealing(float *paa, isax_node *node,
        isax_index *index, float bsf, pqueue_t **pq, pthread_mutex_t *lock_queue, localStack *workstack, int *tnumber) {
    float distance = minidist_paa_to_isax(paa, node->isax_values,
            node->isax_cardinalities,
            index->settings->sax_bit_cardinality,
            index->settings->sax_alphabet_cardinality,
    ...
    if (distance < bsf) {
        if (node->is_leaf) {
            ...
            *tnumber = (*tnumber + 1) % N_PQUEUE;
            ...
            if (node->left_child->isax_cardinalities != NULL) {
                insert_tree_node_m_hybridpqueue_workstealing(paa, node->left_child, index, bsf, pq, lock_queue,
                        workstack, tnumber);
            }
        }
    }
...
void insert_tree_node_m_hybridpqueue(float *paa, isax_node *node,
...
#if defined(__x86_64__)
        if (index->settings->SIMD_flag) {
            distance = __builtin_cpu_supports("avx512vl") ?
                    minidist_fft_to_sfa_rawe_SIMD512(index, paa, node->isax_values, node->isax_cardinalities, bsf) :
                    minidist_fft_to_sfa_rawe_SIMD(index, paa, node->isax_values, node->isax_cardinalities, bsf);
        } else {
#endif
...
#if defined(__x86_64__)
        if (index->settings->SIMD_flag) {
            distance = __builtin_cpu_supports("avx512dq") ?
                    minidist_paa_to_isax_rawa_SIMD512(paa, node->isax_values,
                            node->isax_cardinalities,
                            index->settings->sax_bit_cardinality,
                            index->settings->sax_alphabet_cardinality,
                            index->settings->paa_segments,
                            MINVAL, MAXVAL,
                            index->settings->mindist_sqrt) :
                    minidist_paa_to_isax_rawa_SIMD(paa, node->isax_values,
                            node->isax_cardinalities,
                            index->settings->sax_bit_cardinality,
                            index->settings->sax_alphabet_cardinality,
                            index->settings->paa_segments,
...
void insert_tree_node_m_hybridpqueue_time(float *paa, isax_node *node, isax_index *index, float bsf, pqueue_t **pq,
        pthread_mutex_t *lock_queue, int *tnumber, unsigned long int *time_lb) {
    float distance;
