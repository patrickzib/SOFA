                            ...
                            float minimum_distance, int limit) 
{
    ...
    while (current_root_node != NULL) {
        ...
        {
            mindist_result->distance = minidist_fft_to_isax_complete(index, paa, current_root_node->isax_values, current_root_node->isax_cardinalities);
        }
        else
        {
            ...
            if (n->node->is_leaf) {
                ...
                if (n->node->left_child->isax_cardinalities != NULL) {
                    if(n->node->left_child->is_leaf && !n->node->left_child->has_partial_data_file && aggressive_check){
                        ...
                        if(index->settings->function_type == 4)
                        {
                            mindist_result->distance = minidist_fft_to_isax_complete(index, paa, n->node->left_child->isax_values, n->node->left_child->isax_cardinalities);
                        }
                        else
                        {
                ...
                if (n->node->right_child->isax_cardinalities != NULL) {
                    if(n->node->right_child->is_leaf && !n->node->left_child->has_partial_data_file && aggressive_check){
                        ...
                        if(index->settings->function_type == 4)
                        {
                            mindist_result->distance = minidist_fft_to_isax_complete(index, paa, n->node->right_child->isax_values, n->node->right_child->isax_cardinalities);
                        }
                        else
                        {
...
{   
    ...
    if(index->settings->function_type == 4)
    {
        distance =  minidist_fft_to_isax_complete(index, paa, node->isax_values, node->isax_cardinalities);
    }
    else
    {
