...
void *compute_mindists(void *ptr) {
    ...
    for (uint32_t i = arguments->from; i < arguments->to; i++) {
...
query_result exact_search_serial(ts_type *ts, ts_type *paa, isax_index *index,
...
pqueue_bsf exact_topk_serial(ts_type *ts, ts_type *paa, isax_index *index,
...
void refine_topk_answer(ts_type *ts, ts_type *paa, isax_index *index,
        pqueue_bsf *pq_bsf, float minimum_distance, int limit) {
    int tight_bound = index->settings->tight_bound;
    int aggressive_check = index->settings->aggressive_check;
...
pqueue_bsf exact_topk(ts_type *ts, ts_type *paa, isax_index *index,
        float minimum_distance, int min_checked_leaves, int k) {
    pqueue_bsf *pq_bsf = pqueue_bsf_init(k);
    approximate_topk(ts, paa, index, pq_bsf);
    int tight_bound = index->settings->tight_bound;
...
query_result sanity_check_query(ts_type *ts, ts_type *paa, isax_index *index,
        float minimum_distance, int min_checked_leaves) {
    ...
    while ((n = (query_result*)pqueue_pop(pq))) {
        ...
        if (n->node->is_leaf) {
            ...
            if (distance < tight_mindist) {
                ...
                char *filename = (char*)malloc((strlen(node->filename) + 10) * sizeof(char));
                ...
                int i;
                ts_type *ts_read = (ts_type*)malloc(index->settings->ts_byte_size);
                ts_type *min_wedge = (ts_type*)malloc(index->settings->ts_byte_size);
                ts_type *max_wedge = (ts_type*)malloc(index->settings->ts_byte_size);
