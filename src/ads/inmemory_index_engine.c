...
void index_generate_inmemory(const char *ifilename, long int ts_num, isax_index *index) {
    ...
    rawfile = (float*)malloc(sizeof(ts_type) * index->settings->timeseries_size * ts_num);
    index->sax_cache = (sax_type*)malloc(sizeof(sax_type) * index->settings->paa_segments * ts_num);
    ...
    if (index->settings->SIMD_flag) {
        while (ts_loaded < ts_num) {
            ...
            if (sax_from_ts_SIMD(ts, sax, index->settings->ts_values_per_paa_segment,
...
void index_generate_inmemory_m(const char *ifilename, long int ts_num, isax_index *index) {
    ...
    buffer_data_inmemory *input_data = (buffer_data_inmemory*)malloc(sizeof(buffer_data_inmemory) * (maxquerythread));
    rawfile = (float*)malloc(sizeof(ts_type) * index->settings->timeseries_size * ts_num);
    index->sax_cache = (sax_type*)malloc(sizeof(sax_type) * index->settings->paa_segments * ts_num);
    index->settings->raw_filename = (char*)malloc(256);
    strcpy(index->settings->raw_filename, ifilename);
...
#pragma omp parallel for num_threads(maxquerythread)
    for (unsigned long j = 0; j < ts_num; j++) {
        ...
        if (sax_from_ts(&(rawfile[j * index->settings->timeseries_size]), sax,
            ...
            fbl_soft_buffer2 *current_buffer = &((first_buffer_layer2 *)(
                    index->fbl))->soft_buffers[(int) first_bit_mask];
...
#pragma omp parallel for num_threads(maxquerythread)
    for (int i = 0; i < ((first_buffer_layer2 *)(index->fbl))->number_of_buffers; i++) {
        ...
        if (current_buffer->max_buffer_size != 0) {
            ...
            current_buffer->pos_records = (file_position_type*)malloc(sizeof(file_position_type) *
...
#pragma omp parallel for num_threads(maxquerythread)
    for (int i = 0; i < ((first_buffer_layer2 * )(index->fbl))->number_of_buffers; i++) {
        ...
        if (current_buffer->max_buffer_size != 0) {
            current_buffer->initialized = 1;
            current_buffer->sax_records = (sax_type*)malloc(sizeof(sax_type) * index->settings->paa_segments *
                                                 current_buffer->max_buffer_size);
            current_buffer->pos_records = (file_position_type*)malloc(sizeof(file_position_type) *
                                                 current_buffer->max_buffer_size);
            current_buffer->node = isax_root_node_init((root_mask_type) i,
...
void index_creation_pRecBuf(const char *ifilename, long int ts_num, int filetype_int, int apply_znorm, isax_index *index) {
    ...
    if (filetype_int) {
        rawfile_int32 = (file_type*)malloc(sizeof(file_type) * index->settings->timeseries_size * ts_num);
    }
...
#if defined(__x86_64__)
        if (index->settings->SIMD_flag && __builtin_cpu_supports("avx512f")) {
            for (long int i = 0; i < index->settings->timeseries_size * ts_num;
                    i += index->settings->timeseries_size) {
                znorm_SIMD512F(&rawfile[i], index->settings->timeseries_size);
            }
        } else if (index->settings->SIMD_flag) {
            for (long int i = 0; i < index->settings->timeseries_size * ts_num;
                    i += index->settings->timeseries_size) {
                znorm_SIMD(&rawfile[i], index->settings->timeseries_size);
            }
        } else {
#endif
            for (long int i = 0; i < index->settings->timeseries_size * ts_num;
...
void *index_creation_worker_inmemory_new(void *transferdata) {
    ...
    while (1) {
        ...
        if (current_fbl_node->buffer_size > 0) {
            for (int k = 0; k < current_fbl_node->buffer_size; k++) {
                r->sax = (sax_type *) current_fbl_node->sax_records[k];
                r->position = (file_position_type *) current_fbl_node->pos_records[k];
                r->insertion_mode = PARTIAL_OR_NO_TMP;
                // Add record to index
...
unsigned long cpy_ts_to_fftw_SIMD512F(unsigned long todo, ts_type *ts_fftw, int *j, ts_type *ts) {
    while (todo >= 16) {
        todo -= 16;
        _mm512_storeu_ps(&ts_fftw[*j], _mm512_loadu_ps(&ts[*j]));
        *j += 16;
    }
...
void *index_creation_pRecBuf_worker(void *transferdata) {
    ...
    if (index->settings->function_type == 4) {
        ...
        ts_fftw = (ts_type*)fftwf_malloc(sizeof(ts_type) * ts_length);
        ...
        if (index->settings->function_type == 4) {
...
#if defined(__x86_64__)
            if (index->settings->SIMD_flag) {
                unsigned long todo = ts_length;

                if (__builtin_cpu_supports("avx512f")) {
                    todo = cpy_ts_to_fftw_SIMD512F(todo, ts_fftw, &j, ts);
                }

                while (todo >= 8) {
                    todo -= 8;
                    _mm256_storeu_ps(&ts_fftw[j], _mm256_loadu_ps(&ts[j]));
                    j += 8;
                }
            }
#endif
            ...
            if (sfa_from_ts(index, ts_fftw, sax, ts_out, transform, plan_forward) == SUCCESS) {
                ...
                *pos = (file_position_type) (i * ts_length);
                ...
                fprintf(stderr, "error: cannot insert record in index, since sfa representation failed to be created");
            }
        } else {  // MESSI iSAX
            gettimeofday(&transformation_time_start, NULL);
...
root_mask_type isax_fbl_index_insert_inmemory_para(isax_index *index,
                                                   sax_type *sax,
                                                   file_position_type *pos, pthread_mutex_t *lock_fbl,
...
root_mask_type isax_pRecBuf_index_insert_inmemory(isax_index *index,
        sax_type *sax, file_position_type *pos, pthread_mutex_t *lock_firstnode,
...
root_mask_type isax_fbl_index_insert_inmemory(isax_index *index,
        sax_type *sax, file_position_type *pos) {
    // Create mask for the first bit of the sax representation
...
isax_node *add_record_to_node_inmemory(isax_index *index,
        isax_node *tree_node, isax_node_record *record, const char leaf_size_check) {
#ifdef DEBUG
