...
void isax_index_binary_file_m(const char *ifilename, int ts_num, isax_index *index, int calculate_thread) {
    ...
    int conter = 0;
    long long int i;
    int prev_flush_time = 0;
    int now_flush_time = 0;
    int sax_save_number;
    ...
    ts_type *ts     = (ts_type*)malloc(sizeof(ts_type) *
            index->settings->timeseries_size*read_block_length*(calculate_thread-1));
    ts_type *ts1    = (ts_type*)malloc(sizeof(ts_type) *
            index->settings->timeseries_size*read_block_length*(calculate_thread-1));
    ts_type *ts2;
    sax_type *saxv  = (sax_type*)malloc(sizeof(sax_type) *
            index->settings->paa_segments*read_block_length*(calculate_thread-1));
    sax_type *saxv1 = (sax_type*)malloc(sizeof(sax_type) *
            index->settings->paa_segments*read_block_length*(calculate_thread-1));
    sax_type *saxv2;
    ...
    *pos = ftell(ifile);
    ...
    if (ts_num > read_block_length * (calculate_thread - 1)) {
        COUNT_INPUT_TIME_START
        fread(ts1, sizeof(ts_type),
                index->settings->timeseries_size * read_block_length * (calculate_thread - 1), ifile);
        COUNT_INPUT_TIME_END
        ts2   = ts;
        ts    = ts1;
        ts1   = ts2;
    ...
    *pos = ftell(ifile);
    fread(ts1, sizeof(ts_type),
            index->settings->timeseries_size * (ts_num%(read_block_length * (calculate_thread - 1))), ifile);
    COUNT_INPUT_TIME_END
    ts2   = ts;
    ts    = ts1;
    ts1   = ts2;
...
void isax_index_binary_file_m_new(const char *ifilename, int ts_num, isax_index *index, int calculate_thread) {
    ...
    *pos = ftell(ifile);
    ...
    if (ts_num > read_block_length * (calculate_thread - 1)) {
        ...
        for (i = read_block_length * (calculate_thread - 1) * 2;
                i <= ts_num; i += read_block_length * (calculate_thread - 1)) {
            *pos = ftell(ifile);
...
void isax_index_binary_file_pRecBuf(const char *ifilename, int ts_num, isax_index *index, int calculate_thread) {
    ...
    *pos = ftell(ifile);
    ...
    if (ts_num > read_block_length * (calculate_thread - 1)) {
        ...
        for (i = read_block_length * (calculate_thread-1);
                i < ts_num; i += read_block_length * (calculate_thread - 1)) {
            *pos = ftell(ifile);
...
void isax_index_binary_file_2RecBuf(const char *ifilename, int ts_num, isax_index *index, int calculate_thread) {
    ...
    ifile = fopen(ifilename, "rb");
    index_buffer_data *input_data = (index_buffer_data*)malloc(sizeof(index_buffer_data) * (calculate_thread / 2 + propotion_number));
    file_position_type *pos = (file_position_type*)malloc(sizeof(file_position_type));
    ...
    pthread_barrier_t lock_barrier2;
    pthread_barrier_t lock_barrier3;
    pthread_barrier_t lock_barrier4;
    ...
    ts_type *ts     = (ts_type*)malloc(sizeof(ts_type) *
            index->settings->timeseries_size * read_block_length * (calculate_thread / 2 + propotion_number));
    ts_type *ts1    = (ts_type*)malloc(sizeof(ts_type) *
            index->settings->timeseries_size * read_block_length * (calculate_thread / 2 + propotion_number));
    ts_type *ts2;
    sax_type *saxv  = (sax_type*)malloc(sizeof(sax_type) *
            index->settings->paa_segments * read_block_length * (calculate_thread / 2 + propotion_number));
    sax_type *saxv1 = (sax_type*)malloc(sizeof(sax_type) *
            index->settings->paa_segments * read_block_length * (calculate_thread / 2 + propotion_number));
    sax_type *saxv2;
    ...
    *pos = ftell(ifile);
    ...
    if (ts_num > read_block_length * (calculate_thread / 2 + propotion_number)) {
        ...
        for (i = read_block_length * (calculate_thread /2 + propotion_number);
                i < ts_num; i += read_block_length * (calculate_thread / 2 + propotion_number)) {
            *pos = ftell(ifile);

            // read the data of next round
            fread(ts1, sizeof(ts_type),
                    index->settings->timeseries_size * read_block_length * (calculate_thread / 2 + propotion_number), ifile);
        ...
        *pos = ftell(ifile);
        fread(ts1, sizeof(ts_type),
                index->settings->timeseries_size * (ts_num % (read_block_length * (calculate_thread / 2 + propotion_number))), ifile);
        if (sax_fist_time_check)  {
            COUNT_OUTPUT_TIME_START
            fwrite(saxv1, index->settings->sax_byte_size, read_block_length * (calculate_thread / 2 + propotion_number), index->sax_file);
            COUNT_OUTPUT_TIME_END
        ...
        sax_save_number = conter_ts_number;
        int threadnumber = 0;
        pthread_barrier_t lock_barrier1_bas;
        pthread_barrier_t lock_barrier2_bas;
        ...
        if (sax_fist_time_check) {
            COUNT_OUTPUT_TIME_START
            fwrite(saxv, index->settings->sax_byte_size,
                    ts_num % read_block_length * (calculate_thread / 2 + propotion_number), index->sax_file);
            COUNT_OUTPUT_TIME_END
        } else {
            sax_fist_time_check = true;
        }
...
void isax_index_binary_file_2nRecBuf(const char *ifilename, int ts_num, isax_index *index, int calculate_thread) {
    ...
    int *bufferpresize = (int*)malloc(sizeof(int)
            * ((parallel_dfirst_buffer_layer*)index->fbl)->number_of_buffers * (calculate_thread / 2 + propotion_number) * 2);

    for (long int i = 0;
            i < ((parallel_dfirst_buffer_layer*)index->fbl)->number_of_buffers * (calculate_thread / 2 + propotion_number) * 2;
            i++) {
        bufferpresize[i] = 0;
    }
    ...
    if (ts_num > read_block_length * (calculate_thread / 2 + propotion_number)) {
        ...
        for (i = read_block_length * (calculate_thread / 2 + propotion_number);
            ...
            *pos = ftell(ifile);
            ...
            if (sax_fist_time_check) {
                COUNT_OUTPUT_TIME_START
                fwrite(saxv1, index->settings->sax_byte_size,
                        read_block_length * (calculate_thread / 2 + propotion_number), index->sax_file);
                COUNT_OUTPUT_TIME_END
            } else {
                sax_fist_time_check = true;
            }
...
void* indexbulkloadingworker(void *transferdata) {
    ...
    while (!((index_buffer_data*)transferdata)->finished) {
        ...
        for (i = 0; i < fin_number; i++) {
            if (sax_from_ts(((ts_type*)(((index_buffer_data*)transferdata)->ts) + i * index->settings->timeseries_size),
                    sax, index->settings->ts_values_per_paa_segment,
                ...
                *pos = ((index_buffer_data*)transferdata)->pos + index->settings->timeseries_size * sizeof(ts_type) * i;
                ...
                isax_fbl_index_insert_m(index, sax, pos,
                        ((index_buffer_data*)transferdata)->lock_record, ((index_buffer_data*)transferdata)->lock_fbl,
                        ((index_buffer_data*)transferdata)->lock_cbl, ((index_buffer_data*)transferdata)->lock_firstnode,
                        ((index_buffer_data*)transferdata)->lock_index, ((index_buffer_data*)transferdata)->lock_disk);
            } else {
    ...
    while (!((index_buffer_data*)transferdata)->finished) {
        ...
        for (i = 0; i < fin_number; i++) {
            if (sax_from_ts(((ts_type*)(((index_buffer_data*)transferdata)->ts) + i * index->settings->timeseries_size),
                ...
                *pos = ((index_buffer_data*)transferdata)->pos + index->settings->timeseries_size * sizeof(ts_type) * i;
                memcpy(&(saxv[offset_saxv+i*paa_segments]), sax, sizeof(sax_type) * paa_segments);
                isax_fbl_index_insert_m_new(index, sax, pos,
                        ((index_buffer_data*)transferdata)->lock_record, ((index_buffer_data*)transferdata)->lock_fbl,
                        ((index_buffer_data*)transferdata)->lock_cbl, ((index_buffer_data*)transferdata)->lock_firstnode,
                        ((index_buffer_data*)transferdata)->lock_index, ((index_buffer_data*)transferdata)->lock_disk);
            } else {
    ...
    while (!((index_buffer_data*)transferdata)->finished) {
        ...
        while (1) {
            ...
            for (i = ((index_buffer_data*)transferdata)->bufferpresize[j]; i < current_fbl_node->buffer_size; i++) {
                ...
                r->insertion_mode = PARTIAL_OR_NO_TMP;
    ...
    while (!((index_buffer_data*)transferdata)->finished) {
        ...
        for (int i = 0; i < fin_number; i++) {
            if (sax_from_ts(((ts_type*)(((index_buffer_data*)transferdata)->ts) + i * index->settings->timeseries_size),
                ...
                *pos = ((index_buffer_data*)transferdata)->pos + index->settings->timeseries_size * sizeof(ts_type) * i;
                memcpy(&(saxv[offset_saxv+i*paa_segments]), sax, sizeof(sax_type) * paa_segments);
                isax_pRecBuf_index_insert(index, sax, pos,
                        ((index_buffer_data*)transferdata)->lock_record, ((index_buffer_data*)transferdata)->lock_firstnode,
                        ((index_buffer_data*)transferdata)->workernumber, ((index_buffer_data*)transferdata)->total_workernumber);
            } else {
                fprintf(stderr, "error: cannot insert record in index, since sax representation failed to be created");
            }
        }
...
void* indexbulkloadingworker_2RecBuf(void *transferdata) {
    ...
    while (!((index_buffer_data*)transferdata)->finished) {
        ...
        for (i = 0; i < fin_number; i++) {
            if (sax_from_ts((ts + i * index->settings->timeseries_size), sax,
                ...
                *pos = readpos + index->settings->timeseries_size * sizeof(ts_type) * i;
                memcpy(&(saxv[offset_saxv+i*paa_segments]), sax, sizeof(sax_type) * paa_segments);
                isax_2pRecBuf_index_insert(index, sax, pos,
                        ((index_buffer_data*)transferdata)->lock_record, ((index_buffer_data*)transferdata)->lock_firstnode,
...
isax_node * insert_to_fbl_m(first_buffer_layer *fbl, sax_type *sax, file_position_type *pos, root_mask_type mask,
    ...
    if (current_buffer->buffer_size >= current_buffer->max_buffer_size) {
        if (current_buffer->max_buffer_size == 0) {
            ...
            current_buffer->pos_records =
                    (file_position_type**)malloc(sizeof(file_position_type *) * current_buffer->max_buffer_size);
        } else {
            current_buffer->max_buffer_size *= BUFFER_REALLOCATION_RATE;
            current_buffer->sax_records = (sax_type**)realloc(current_buffer->sax_records,
                    sizeof(sax_type *) * current_buffer->max_buffer_size);
            current_buffer->pos_records = (file_position_type**)realloc(current_buffer->pos_records,
                    sizeof(file_position_type *) * current_buffer->max_buffer_size);
        }
    }
...
isax_node *insert_to_fbl_m_new(first_buffer_layer *fbl, sax_type *sax, file_position_type *pos,
        root_mask_type mask, isax_index *index, pthread_mutex_t *lock_firstnode, pthread_mutex_t *lockfbl) {
    fbl_soft_buffer *current_buffer = &fbl->soft_buffers[(int) mask];
    char *cd_s;
    char *cd_p;
...
isax_node * insert_to_pRecBuf(parallel_first_buffer_layer *fbl, sax_type *sax,
        file_position_type *pos, root_mask_type mask, isax_index *index,
        pthread_mutex_t *lock_firstnode, int workernumber, int total_workernumber) {
...
isax_node * insert_to_2pRecBuf(parallel_dfirst_buffer_layer *fbl, sax_type *sax, file_position_type *pos,
        root_mask_type mask, isax_index *index, pthread_mutex_t *lock_firstnode, int workernumber, int total_workernumber) {
    ...
    if (current_buffer->buffer_size[workernumber] >= current_buffer->max_buffer_size[workernumber]) {
        if (current_buffer->max_buffer_size[workernumber] == 0) {
            current_buffer->max_buffer_size[workernumber] = fbl->initial_buffer_size;
            current_buffer->sax_records[workernumber] = (sax_type**)malloc(sizeof(sax_type *) *
                    current_buffer->max_buffer_size[workernumber]);
            current_buffer->pos_records[workernumber] = (file_position_type**)malloc(sizeof(file_position_type *) *
                    current_buffer->max_buffer_size[workernumber]);
        } else {
            current_buffer->max_buffer_size[workernumber] *= BUFFER_REALLOCATION_RATE;
            current_buffer->sax_records[workernumber] = (sax_type**)realloc(current_buffer->sax_records[workernumber],
                    sizeof(sax_type *) * current_buffer->max_buffer_size[workernumber]);
            current_buffer->pos_records[workernumber] = (file_position_type**)realloc(current_buffer->pos_records[workernumber],
                    sizeof(file_position_type *) * current_buffer->max_buffer_size[workernumber]);
        }
    }
    ...
    filepointer = (file_position_type **)current_buffer->pos_records[workernumber];
    saxpointer = (sax_type **)current_buffer->sax_records[workernumber];
    saxpointer[current_buffer_number] =
            (sax_type*)((parallel_dfirst_buffer_layer*)index->fbl)->hard_buffer +
             *pos / sizeof(ts_type) / index->settings->timeseries_size * index->settings->sax_byte_size;
    filepointer[current_buffer_number] =
            (file_position_type *)(((parallel_dfirst_buffer_layer*)index->fbl)->hard_buffer +
                    *pos / sizeof(ts_type)/index->settings->timeseries_size * index->settings->sax_byte_size + index->settings->sax_byte_size);
...
enum response indexconstruction(first_buffer_layer *fbl, isax_index *index,
        pthread_mutex_t *lock_index, pthread_mutex_t *lock_disk, int calculate_thread) {
#ifdef DEBUG
    printf("*** FLUSHING ***\n\n");
#else
#if VERBOSE_LEVEL == 2
...
enum response indexflush(first_buffer_layer *fbl, isax_index *index,
        pthread_mutex_t *lock_index, pthread_mutex_t *lock_disk, int calculate_thread) {
#ifdef DEBUG
    printf("*** FLUSHING ***\n\n");
#else
...
enum response indexpRecBufflush(parallel_first_buffer_layer *fbl, isax_index *index,
        pthread_mutex_t *lock_index, pthread_mutex_t *lock_disk, int calculate_thread, int preworkernumber) {
#ifdef DEBUG
    printf("*** FLUSHING ***\n\n");
#else
#if VERBOSE_LEVEL == 2
...
enum response indexconstruction_pRecBuf(first_buffer_layer *fbl, isax_index *index,
...
void* indexpRecBufflushworker(void *input) {
    ...
    while (1) {
        ...
        if (have_record) {
            ...
            // clear records read from files (free only prev sax buffers)
            isax_index_clear_node_buffers(index, current_fbl_node->node, INCLUDE_CHILDREN, TMP_AND_TS_CLEAN);
            index->allocated_memory = 0;  // Set to 0 in order to re-allocate original space for buffers.
        }
    }
}
    ...
    while (!((trans_fbl_input*)input)->finished) {
        ...
        while (1) {
            ...
            if (current_fbl_node->buffer_size[fbloffset] > 0) {
                for (i = ((trans_fbl_input*)input)->buffersize[j + fbloffset * index->fbl->number_of_buffers];
                        i < current_fbl_node->buffer_size[fbloffset]; i++) {
    ...
    while (!((trans_fbl_input*)input)->finished) {
        ...
        while (1) {
            ...
            for (k = fbloffset; k < fbloffset + preworkernumber; k++) {
                for (int i = ((trans_fbl_input*)input)->buffersize[j + k * index->fbl->number_of_buffers];
                        i < current_fbl_node->buffer_size[k]; i++) {
...
root_mask_type isax_fbl_index_insert_m(isax_index *index, sax_type * sax, file_position_type * pos,
        pthread_mutex_t *lock_record, pthread_mutex_t *lock_fbl, pthread_mutex_t *lock_cbl,
        pthread_mutex_t *lock_firstnode, pthread_mutex_t *lock_index, pthread_mutex_t *lock_disk) {
    int i;
    int t;
...
root_mask_type isax_fbl_index_insert_m_new(isax_index *index, sax_type * sax, file_position_type * pos,
        pthread_mutex_t *lock_record, pthread_mutex_t *lock_fbl, pthread_mutex_t *lock_cbl,
        pthread_mutex_t *lock_firstnode, pthread_mutex_t *lock_index, pthread_mutex_t *lock_disk) {
    int i;
    int t;
...
root_mask_type isax_pRecBuf_index_insert(isax_index *index, sax_type * sax, file_position_type * pos,
        pthread_mutex_t *lock_record, pthread_mutex_t *lock_firstnode, int workernumber, int total_workernumber) {
    int i;
    int t;
    int totalsize = index->settings->max_total_buffer_size;
    // Create mask for the first bit of the sax representation
    ...
    root_mask_type first_bit_mask = 0x00;
    CREATE_MASK(first_bit_mask, index, sax);
    insert_to_pRecBuf((parallel_first_buffer_layer*)(index->fbl), sax, pos, first_bit_mask,
            index, lock_firstnode, workernumber, total_workernumber);

    return first_bit_mask;
}

root_mask_type isax_2pRecBuf_index_insert(isax_index *index, sax_type * sax, file_position_type * pos,
        pthread_mutex_t *lock_record, pthread_mutex_t *lock_firstnode, int workernumber, int total_workernumber) {
    int i;
    int t;
    int totalsize = index->settings->max_total_buffer_size;
    ...
    memcpy(current_recode + *pos / sizeof(ts_type) / index->settings->timeseries_size * index->settings->sax_byte_size,
            sax, index->settings->sax_byte_size);
    memcpy(current_recode + *pos / sizeof(ts_type) / index->settings->timeseries_size *
                index->settings->sax_byte_size + index->settings->sax_byte_size, pos, index->settings->position_byte_size);
    insert_to_2pRecBuf((parallel_dfirst_buffer_layer*)(index->fbl), sax, pos, first_bit_mask,
            index, lock_firstnode, workernumber, total_workernumber);
