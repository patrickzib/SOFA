...
void calculate_average_depth(FILE *ifile, isax_index *index) {
    ...
    char *buffer_number = (char*)malloc(BUFFER_SIZE * sizeof(char));
    char *buffer_depth = (char*)malloc(BUFFER_SIZE * sizeof(char));
    char *buffer_leaf_size = (char*)malloc(BUFFER_SIZE * sizeof(char));

    char logfile_out_header[STRING_SIZE] = "subtrees,average depth,average leaf size\n";
    char logfile_out_values[STRING_SIZE] = "";

    double depth = 0.0;
    uint32_t leaf_size_total = 0;
...
void traverse_tree(isax_node *node, unsigned int parent_depth, unsigned int *total_depth,
        unsigned int *node_count, unsigned int *leaf_count, unsigned long *leaf_size) {
    if (node->is_leaf) {
        *leaf_count += 1;
    ...
    if (node->left_child) {
        traverse_tree(node->left_child, ++parent_depth, total_depth,
                node_count, leaf_count, leaf_size);
    }

    if (node->right_child) {
        traverse_tree(node->right_child, ++parent_depth, total_depth,
                node_count, leaf_count, leaf_size);
    }
