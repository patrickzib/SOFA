...
enum response paa_from_ts(ts_type *ts_in, ts_type *paa_out, int segments,
    ...
    for (int s = 0; s < segments; s++) {
        ...
        left_values -= ts_values_per_segment;
    }
...
enum response sax_from_ts_SIMD(ts_type *ts_in, sax_type *sax_out, int ts_values_per_segment,
    ...
    for (int s = 0; s < segments; s++) {
        ...
        for (int i = 8; i < ts_values_per_segment; i += 8) {
...
float minidist_paa_to_isax(float *paa, sax_type *sax,
    ...
    for (int i = 0; i < number_of_segments; i++) {
        ...
        float breakpoint_lower;  // <-- TODO(someone): calculate breakpoints.
...
float minidist_paa_to_isax_SIMD(float *paa, sax_type *sax,
    ...
    __m256 breakpoint_upperv_0 = (__m256) _mm256_or_si256(
            _mm256_and_si256(upper_juge_maxv_0, (__m256i) _mm256_set1_ps(max_val)),
            _mm256_and_si256(upper_juge_nmaxv_0, (__m256i) usax_breakpoints_shiftv_0));
    __m256 breakpoint_upperv_1 = (__m256) _mm256_or_si256(
            _mm256_and_si256(upper_juge_maxv_1, (__m256i) _mm256_set1_ps(max_val)),
...
#if defined(__x86_64__)
float minidist_paa_to_isax_raw_SIMD(float *paa, sax_type *sax, sax_type *sax_cardinalities, sax_type max_bit_cardinality,
        int max_cardinality, int number_of_segments, int min_val, int max_val, float ratio_sqrt) {
    int region_upper[16];
    int region_lower[16];
    float distancef[16];
    int offset = ((max_cardinality - 1) * (max_cardinality - 2)) / 2;
...
float minidist_paa_to_isax_rawa_SIMD(float *paa, sax_type *sax,
    ...
    __m256i vupperoffset = _mm256_set1_epi32(offset);
    __m256 minvalv = _mm256_set1_ps(min_val);
    __m256 maxvalv = _mm256_set1_ps(max_val);
    ...
    __m256 breakpoint_upperv_0 = (__m256) _mm256_or_si256(
            _mm256_and_si256(upper_juge_maxv_0, (__m256i)maxvalv),
            _mm256_and_si256(upper_juge_nmaxv_0, (__m256i) usax_breakpoints_shiftv_0));
    __m256 breakpoint_upperv_1 = (__m256) _mm256_or_si256(
            _mm256_and_si256(upper_juge_maxv_1, (__m256i)maxvalv),
            _mm256_and_si256(upper_juge_nmaxv_1, (__m256i) usax_breakpoints_shiftv_1));
...
float minidist_paa_to_isax_rawa_SIMD512(float *paa, sax_type *sax,
        sax_type *sax_cardinalities,
        sax_type max_bit_cardinality,
        int max_cardinality,
        int number_of_segments,
    ...
    __m256 minvalv = _mm256_set1_ps(min_val);
    __m256 maxvalv = _mm256_set1_ps(max_val);
    __m512i vloweroffsetf = _mm512_set1_epi32(offset - 1);
    __m512i vupperoffsetf = _mm512_set1_epi32(offset);
    __m512 minvalvf = _mm512_set1_ps(min_val);
    __m512 maxvalvf = _mm512_set1_ps(max_val);
    __m512i vectorsignbitf = _mm512_set1_epi32(0xffffffff);

    __m128i sax_cardinalitiesv8 = _mm_lddqu_si128((const __m128i*) sax_cardinalities);
    __m512i c_cv = _mm512_cvtepu8_epi32(sax_cardinalitiesv8);

    __m128i saxv8 = _mm_lddqu_si128((const __m128i*) sax);
    __m512i saxv = _mm512_cvtepu8_epi32(saxv8);

    __m512i c_mf = _mm512_set1_epi32(max_bit_cardinality);
    __m512i cm_ccv = _mm512_sub_epi32(c_mf, c_cv);

    __m512i region_lowerv = _mm512_srlv_epi32(saxv, cm_ccv);
    region_lowerv = _mm512_sllv_epi32(region_lowerv, cm_ccv);

    __m512i region_upperv = _mm512_sllv_epi32(vectorsignbitf, cm_ccv);
    region_upperv = _mm512_andnot_si512(region_upperv, vectorsignbitf);

    region_upperv = _mm512_or_si512(region_upperv, region_lowerv);

    __m512i region_lowerv_offset = _mm512_add_epi32(region_lowerv, vloweroffsetf);
    __m512i region_upperv_offset = _mm512_add_epi32(region_upperv, vloweroffsetf);

    // lower
    __mmask16 lower_juge_zero_mask = _mm512_cmpeq_epi32_mask(region_lowerv, _mm512_setzero_si512());

    __mmask16 lower_juge_nzero_mask = _mm512_kandn(lower_juge_nzero_mask, 0xffff);

    __m512 lsax_breakpoints_shiftv = _mm512_i32gather_ps(region_lowerv_offset, sax_breakpoints,  4);

    __m512 breakpoint_lowerv = (__m512) _mm512_or_si512(
            _mm512_maskz_mov_epi32(lower_juge_zero_mask, (__m512i)minvalvf),
            _mm512_maskz_mov_epi32(lower_juge_nzero_mask, (__m512i)lsax_breakpoints_shiftv));

    // uper
    __m512 usax_breakpoints_shiftv = _mm512_i32gather_ps(region_upperv_offset, sax_breakpoints, 4);

    __mmask16 upper_juge_max_mask = _mm512_cmpeq_epi32_mask(region_upperv, _mm512_set1_epi32(max_cardinality - 1));

    __mmask16 upper_juge_nmax_mask = _mm512_kandn(upper_juge_max_mask, 0xffff);

    __m512 breakpoint_upperv = (__m512)_mm512_or_si512(
            _mm512_maskz_mov_epi32(upper_juge_max_mask, (__m512i)maxvalvf),
            _mm512_maskz_mov_epi32(upper_juge_nmax_mask, (__m512i)usax_breakpoints_shiftv));

    // dis
    __m512 paav = _mm512_loadu_ps(paa);

    __mmask16 dis_juge_upv = _mm512_cmp_ps_mask(breakpoint_lowerv, paav, _CMP_GT_OS);

    __mmask16 dis_juge_lov = _mm512_cmp_ps_mask(breakpoint_upperv, paav, _CMP_LT_OS);

    __m512 dis_lowv = _mm512_sub_ps(breakpoint_lowerv, paav);
    __m512 dis_uppv = _mm512_sub_ps(breakpoint_upperv, paav);


     __mmask16 distance = _mm512_kor(dis_juge_lov, dis_juge_upv);
     __m512 distancev = _mm512_or_ps(
             _mm512_maskz_mov_ps(dis_juge_upv, dis_lowv),
             _mm512_maskz_mov_ps(dis_juge_lov, dis_uppv));

     __m256 distancesum_0 = _mm256_dp_ps(_mm512_extractf32x8_ps(distancev, 0), _mm512_extractf32x8_ps(distancev, 0), 0xff);
     __m256 distancesum_1 = _mm256_dp_ps(_mm512_extractf32x8_ps(distancev, 1), _mm512_extractf32x8_ps(distancev, 1), 0xff);
     __m256 distancevf = _mm256_add_ps(distancesum_0, distancesum_1);
