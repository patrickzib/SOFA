...
enum response add_to_node_buffer(isax_node_buffer *node_buffer,  isax_node_record *record,
    ...
    if (record->insertion_mode & TMP)  {
        if (record->insertion_mode & FULL) {
            if (node_buffer->max_tmp_full_buffer_size == 0) {
                ...
                node_buffer->tmp_full_position_buffer = (file_position_type**)realloc(node_buffer->tmp_full_position_buffer,
                        sizeof(file_position_type*) *  node_buffer->max_tmp_full_buffer_size);
                node_buffer->tmp_full_sax_buffer = (sax_type**)realloc(node_buffer->tmp_full_sax_buffer,
                        sizeof(sax_type*) *  node_buffer->max_tmp_full_buffer_size);
                node_buffer->tmp_full_ts_buffer = (ts_type**)realloc(node_buffer->tmp_full_ts_buffer,
                        sizeof(ts_type*) *  node_buffer->max_tmp_full_buffer_size);
            }
        ...
        if (record->insertion_mode & PARTIAL) {
            if (node_buffer->max_tmp_partial_buffer_size == 0) {
                ...
                node_buffer->tmp_partial_position_buffer = (file_position_type**)realloc(node_buffer->tmp_full_position_buffer,
                        sizeof(file_position_type*) * node_buffer->max_tmp_partial_buffer_size);
                node_buffer->tmp_partial_sax_buffer = (sax_type**)realloc(node_buffer->tmp_full_sax_buffer,
                        sizeof(sax_type*) *  node_buffer->max_tmp_partial_buffer_size);
            }
        ...
        if (record->insertion_mode & FULL) {
            if (node_buffer->max_full_buffer_size == 0) {
                ...
                node_buffer->full_sax_buffer = (sax_type**)malloc(sizeof(sax_type*) *  node_buffer->max_full_buffer_size);
                node_buffer->full_ts_buffer = (ts_type**)malloc(sizeof(ts_type*) *  node_buffer->max_full_buffer_size);
                ...
                node_buffer->full_position_buffer = (file_position_type**)realloc(node_buffer->full_position_buffer,
                        sizeof(file_position_type*) *  node_buffer->max_full_buffer_size);
                node_buffer->full_sax_buffer = (sax_type**)realloc(node_buffer->full_sax_buffer,
                        sizeof(sax_type*) *  node_buffer->max_full_buffer_size);
                node_buffer->full_ts_buffer = (ts_type**)realloc(node_buffer->full_ts_buffer,
                        sizeof(ts_type*) *  node_buffer->max_full_buffer_size);
            }
        ...
        if (record->insertion_mode & PARTIAL) {
            if (node_buffer->max_partial_buffer_size == 0) {
                ...
                node_buffer->partial_position_buffer = (file_position_type**)realloc(node_buffer->partial_position_buffer,
                        sizeof(file_position_type*) *  node_buffer->max_partial_buffer_size);
                node_buffer->partial_sax_buffer = (sax_type**)realloc(node_buffer->partial_sax_buffer,
                        sizeof(sax_type*) *  node_buffer->max_partial_buffer_size);
            }
...
enum response flush_node_buffer(isax_node_buffer *node_buffer,
        int sax_segments, int ts_segments,  const char * filename) {
    ...
    if (node_buffer->full_buffer_size > 0  || node_buffer->tmp_full_buffer_size > 0) {
        ...
        for (i = 0; i < node_buffer->full_buffer_size; i++) {
            fwrite(node_buffer->full_position_buffer[i], sizeof(file_position_type), 1, full_file);
            fwrite(node_buffer->full_sax_buffer[i], sizeof(sax_type), sax_segments, full_file);
            fwrite(node_buffer->full_ts_buffer[i], sizeof(ts_type), ts_segments, full_file);
        }

        for (i = 0; i < node_buffer->tmp_full_buffer_size; i++) {
            fwrite(node_buffer->tmp_full_position_buffer[i], sizeof(file_position_type), 1, full_file);
            fwrite(node_buffer->tmp_full_sax_buffer[i],  sizeof(sax_type), sax_segments, full_file);
            fwrite(node_buffer->tmp_full_ts_buffer[i],  sizeof(ts_type), ts_segments, full_file);
        }
    ...
    if (node_buffer->partial_buffer_size > 0 || node_buffer->tmp_partial_buffer_size > 0) {
        ...
        for (i = 0; i < node_buffer->partial_buffer_size; i++) {
            fwrite(node_buffer->partial_position_buffer[i], sizeof(file_position_type), 1, partial_file);
            fwrite(node_buffer->partial_sax_buffer[i],  sizeof(sax_type), sax_segments, partial_file);
        }

        for (i = 0; i < node_buffer->tmp_partial_buffer_size; i++) {
            fwrite(node_buffer->tmp_partial_position_buffer[i], sizeof(file_position_type), 1, partial_file);
            fwrite(node_buffer->tmp_partial_sax_buffer[i], sizeof(sax_type), sax_segments, partial_file);
        }
...
enum response flush_node_buffer_m(isax_node_buffer *node_buffer,
        int sax_segments, int ts_segments, const char * filename, pthread_mutex_t *lock_write) {
    // WRITE TWO DIFFERENT FILES!
    // 1. .FULL (full records)
    // 2. .PART (partial records)
    int i;
