...
isax_index_settings * isax_index_settings_init(const char * root_directory, int timeseries_size,
    ...
    settings->full_record_size = settings->sax_byte_size
            + settings->position_byte_size + settings->ts_byte_size;
    ...
    settings->mindist_sqrt = ((float) settings->timeseries_size / (float) settings->paa_segments);
    ...
    settings->max_filename_size = settings->paa_segments * ((c_size * 2) + 2) + 5 + strlen(root_directory);
    ...
    if (SIMD_flag) {
        ...
        settings->SIMD_flag = false;
    }

    if (is_norm) {
        settings->is_norm = '1';
    } else {
        settings->is_norm = false;
    }
...
isax_index * isax_index_init(isax_index_settings *settings) {
    ...
    index->fbl = initialize_fbl(settings->initial_fbl_buffer_size, pow(2, settings->paa_segments),
...
void isax_index_destroy(isax_index *index, isax_node *node) {
    if (node == NULL) {
        ...
        // TODO(someone): OPTIMIZE TO FLUSH WITHOUT TRAVERSAL!
...
root_mask_type isax_fbl_index_insert(isax_index *index, sax_type * sax, file_position_type * pos) {
    ...
    // TODO(someone): Create INSERTION SHORT AND BINARY SEARCH METHODS.
...
enum response isax_index_insert(isax_index *index, sax_type *sax, file_position_type *pos) {
...
#endif
        isax_node *new_node = isax_root_node_init(first_bit_mask, index->settings->initial_leaf_buffer_size);
        index->root_nodes++;
    ...
    isax_node_record * record = isax_node_record_init(index->settings->paa_segments,
            index->settings->timeseries_size, PARTIAL_OR_NO_TMP);
    ...
    // TODO(someone): use total buffered records, also increased from LEAF LOADS and SPLITS
...
enum response create_node_filename(isax_index *index, isax_node *node, isax_node_record *record) {
    ...
    if (node->parent) {
        for (i = 0; i < index->settings->paa_segments; i++) {
            ...
            for (int k = 0; k <= node->parent->split_data->split_mask[i]; k++) {
    ...
    } else {  // If it has no parent it is root node and as such it's cardinality is 1.
        root_mask_type mask = 0x00;
...
isax_node * add_record_to_node(isax_index *index, isax_node *tree_node,
        isax_node_record *record, const char leaf_size_check) {
#ifdef DEBUG
    ...
    if ((node->leaf_size) >= index->settings->max_leaf_size && leaf_size_check) {
        ...
        add_to_node_buffer(node->buffer, record, index->settings->paa_segments, index->settings->timeseries_size);
        node->leaf_size++;
    }
...
//    isax_index_clear_node_buffers(index, NULL, 1, buffer_clean_mode);
    isax_index_clear_node_buffers(index, NULL, INCLUDE_CHILDREN, buffer_clean_mode);
...
enum response flush_subtree_leaf_buffers(isax_index *index, isax_node *node) {
    if (node->is_leaf && node->filename != NULL) {
        ...
        index->memory_info.disk_data_full += (node->buffer->full_buffer_size + node->buffer->tmp_full_buffer_size);
        index->memory_info.disk_data_partial += (node->buffer->partial_buffer_size + node->buffer->tmp_partial_buffer_size);

        flush_node_buffer(node->buffer, index->settings->paa_segments, index->settings->timeseries_size, node->filename);
...
enum response flush_subtree_leaf_buffers_m1(isax_index *index, isax_node *node,
        pthread_mutex_t *lock_index, pthread_mutex_t *lock_write) {
    if (node->is_leaf && node->filename != NULL) {
        // Set that unloaded data exist in disk
        if (node->buffer->partial_buffer_size > 0 || node->buffer->tmp_partial_buffer_size > 0) {
            node->has_partial_data_file = 1;
        ...
        flush_node_buffer_m(node->buffer, index->settings->paa_segments,
                index->settings->timeseries_size, node->filename, lock_write);
    } else if (!node->is_leaf) {
        pid_t fpid;
        fpid = fork();
        if (fpid < 0) {
...
void find_empty_children(isax_node *start_node, isax_node **nodes_to_load, int * number, int * offset) {
...
printf("*** %pis a leaf, loaded.\n", start_node);
        ...
        if (start_node->left_child != NULL) {
            find_empty_children(start_node->left_child, nodes_to_load, number, offset);
        }

        if (start_node->right_child != NULL) {
            find_empty_children(start_node->right_child, nodes_to_load, number, offset);
        }
    }
}

int find_siblings(isax_node *c_node, isax_node **nodes_to_load, int *number_of_leaves, int *offset) {
    ...
    while (*number_of_leaves > 0 && !(ptr_left == NULL && ptr_right == NULL)) {
        if (ptr_left != NULL) {
            find_empty_children(ptr_left, nodes_to_load, number_of_leaves, offset);
            ptr_left = ptr_left->previous;
        }

        if (ptr_right != NULL) {
            find_empty_children(ptr_right, nodes_to_load, number_of_leaves, offset);
...
float isax_index_load_node(isax_index *index, isax_node *c_node, ts_type * query, float bsf) {
    ...
    char* pfilename = (char*)malloc(255);
    snprintf(pfilename, sizeof(pfilename), "%s.%llu", index->settings->raw_filename, fbl_node->mask);
    ...
    int offset = 0;
    isax_node **nodes_to_load = (isax_node**)malloc(sizeof(isax_node *) * leaves_to_load);
    ...
    for (node_id = 0; node_id < offset; node_id++) {
        ...
        if (node->buffer) {
            buffer_records = node->buffer->partial_buffer_size + node->buffer->tmp_partial_buffer_size;
        ...
        if (load_buffer == NULL) {
            load_buffer = (isax_node_record*)malloc(sizeof(isax_node_record) * (total_records));
        } else {
            load_buffer = (isax_node_record*)realloc(load_buffer, sizeof(isax_node_record) * (total_records));
        }
        ...
        if (node->buffer != NULL) {
            ...
            for (i = 0; i < node->buffer->partial_buffer_size; i++) {
                ...
                load_buffer[load_buffer_index].ts = (ts_type*)malloc(index->settings->ts_byte_size);
                load_buffer[load_buffer_index].insertion_mode = FULL_OR_NO_TMP;
            ...
            for (i = 0; i < node->buffer->tmp_partial_buffer_size; i++) {
                ...
                load_buffer[load_buffer_index].ts = (ts_type*)malloc(index->settings->ts_byte_size);
                load_buffer[load_buffer_index].insertion_mode = TMP_OR_FULL;
                load_buffer[load_buffer_index].destination = node;
                load_buffer_index++;
            }
            ...
            index->allocated_memory += index->settings->ts_byte_size *
                    (node->buffer->tmp_partial_buffer_size + node->buffer->partial_buffer_size);
        }
        ...
        if (file_records > 0) {
            ...
            for (i = 0; i < file_records; i++) {
                ...
                load_buffer[load_buffer_index].position = (file_position_type*)malloc(index->settings->position_byte_size);
                load_buffer[load_buffer_index].sax = (sax_type*)malloc(index->settings->sax_byte_size);
                load_buffer[load_buffer_index].ts = (ts_type*)malloc(index->settings->ts_byte_size);
                load_buffer[load_buffer_index].insertion_mode = TMP_OR_FULL;
                load_buffer[load_buffer_index].destination = node;
                COUNT_INPUT_TIME_START
    ...
    for (int i = 0; i < total_records; i++) {
        ...
        if (query != NULL) {
            ...
            float dist = ts_euclidean_distance(query, load_buffer[i].ts, index->settings->timeseries_size, bsf);
            if (dist < bsf) {
                bsf = dist;
#ifdef STORE_ANSWER
        ...
        add_to_node_buffer(((isax_node *)load_buffer[i].destination)->buffer,
                &load_buffer[i], index->settings->paa_segments, index->settings->timeseries_size);
    }
    ...
    if (!index->has_wedges) {
                ...
                if (fread(pos, sizeof(file_position_type), 1, partial_file)) {
                    if (fread(sax, sizeof(sax_type), index->settings->paa_segments, partial_file)) {
                        float mindist =
                                        ...
                                        minidist_paa_to_isax_raw_SIMD(query, sax, index->settings->max_sax_cardinalities,
                                index->settings->sax_bit_cardinality,
                                index->settings->sax_alphabet_cardinality,
                                index->settings->paa_segments, MINVAL, MAXVAL,
                                index->settings->mindist_sqrt);
#else
                                        minidist_paa_to_isax_raw(query, sax, index->settings->max_sax_cardinalities,
                                index->settings->sax_bit_cardinality,
                                index->settings->sax_alphabet_cardinality,
                                index->settings->paa_segments, MINVAL, MAXVAL,
                                index->settings->mindist_sqrt);
        ...
        if (node->buffer != NULL) {
            for (i = 0; i < node->buffer->partial_buffer_size; i++) {
                float mindist =
#if defined(__x86_64)
                                minidist_paa_to_isax_raw_SIMD(query, node->buffer->partial_sax_buffer[i], index->settings->max_sax_cardinalities,
                        index->settings->sax_bit_cardinality,
                        index->settings->sax_alphabet_cardinality,
                        index->settings->paa_segments, MINVAL, MAXVAL,
                        index->settings->mindist_sqrt);
#else
                                minidist_paa_to_isax_raw(query, node->buffer->partial_sax_buffer[i], index->settings->max_sax_cardinalities,
                        index->settings->sax_bit_cardinality,
                        index->settings->sax_alphabet_cardinality,
                        index->settings->paa_segments, MINVAL, MAXVAL,
                        index->settings->mindist_sqrt);
            ...
            for (i = 0; i < node->buffer->tmp_partial_buffer_size; i++) {
                float mindist =
#if defined(__x86_64__)
                                minidist_paa_to_isax_raw_SIMD(query, node->buffer->tmp_partial_sax_buffer[i], index->settings->max_sax_cardinalities,
                        index->settings->sax_bit_cardinality,
                        index->settings->sax_alphabet_cardinality,
                        index->settings->paa_segments, MINVAL, MAXVAL,
                        index->settings->mindist_sqrt);
#else
                                minidist_paa_to_isax_raw(query, node->buffer->tmp_partial_sax_buffer[i], index->settings->max_sax_cardinalities,
                          index->settings->sax_bit_cardinality,
                          index->settings->sax_alphabet_cardinality,
                          index->settings->paa_segments, MINVAL, MAXVAL,
                          index->settings->mindist_sqrt);
...
float calculate_node_distance(isax_index *index, isax_node *node, ts_type *query, float bsf) {
    ...
    if (node->has_full_data_file) {
        ...
        if (full_file != NULL) {
            ...
            ts_type *ts = (ts_type*)malloc(index->settings->ts_byte_size);
            ...
            while (file_records > 0) {
                ...
                float dist = ts_euclidean_distance(query, ts, index->settings->timeseries_size, bsf);
...
float calculate_node_distance_SIMD(isax_index *index, isax_node *node, ts_type *query, float bsf) {
    ...
    if (node->buffer != NULL) {
        ...
        for (i = 0; i < node->buffer->full_buffer_size; i++) {
            float dist =
#if defined(__x86_64)
                         ts_euclidean_distance_SIMD(query, node->buffer->full_ts_buffer[i],
                    index->settings->timeseries_size, bsf);
#else
                        ts_euclidean_distance(query, node->buffer->full_ts_buffer[i],
                    index->settings->timeseries_size, bsf);
#endif
        ...
        for (i = 0; i < node->buffer->tmp_full_buffer_size; i++) {
            float dist =
#if defined(__x86_64__)
                         ts_euclidean_distance_SIMD(query, node->buffer->tmp_full_ts_buffer[i],
                    index->settings->timeseries_size, bsf);
#else
                        ts_euclidean_distance(query, node->buffer->tmp_full_ts_buffer[i],
                    index->settings->timeseries_size, bsf);
#endif
    ...
    if (node->has_full_data_file) {
        ...
        if (full_file != NULL) {
            ...
            int i = 0;
            ts_type *tsp;
            size_t *ts_buffer = (size_t*)malloc(file_size);
            COUNT_INPUT_TIME_START
            ...
            while (file_records > 0) {
                COUNT_INPUT_TIME_START
                COUNT_INPUT_TIME_END
                tsp = (ts_type*)(ts_buffer + i * index->settings->full_record_size + sizeof(file_position_type) + index->settings->sax_byte_size);
#if defined(__x86_64__)
                float dist = ts_euclidean_distance_SIMD(query, tsp, index->settings->timeseries_size, bsf);
#else
                float dist = ts_euclidean_distance(query, tsp, index->settings->timeseries_size, bsf);
#endif
...
void complete_index_leafs(isax_index *index) {
    ...
    // TODO(someone): CHECK THAT THIS IS NEEDED!
    FLUSHES++;
    flush_all_leaf_buffers(index, TMP_ONLY_CLEAN);
    index->allocated_memory = 0;
}
...
void complete_index(isax_index *index, int ts_num) {
    ...
    int ts_loaded = 0;
    isax_node_record *record = (isax_node_record*)malloc(sizeof(isax_node_record));
    ...
    while (ts_loaded < ts_num) {
        ...
        if (sax_from_ts(ts, sax, index->settings->ts_values_per_paa_segment,
            ...
            CREATE_MASK(first_bit_mask, index, sax);
            record->sax = sax;
            record->position = (file_position_type*)malloc(sizeof(file_position_type));
            record->ts = ts;
            *(record->position) =  ts_loaded * index->settings->ts_byte_size;
            record->insertion_mode = TMP_OR_FULL;
...
void cache_sax_file(isax_index *index) {
    ...
    uint32_t size = ftell(index->sax_file);
    fseek(index->sax_file, 0L, SEEK_SET);
    index->sax_cache = (sax_type*)malloc(size);
...
isax_node *node_read(isax_index *index, FILE *file) {
    ...
    if (is_leaf) {
        node = (isax_node*)malloc(sizeof(isax_node));
        node->is_leaf = 1;
        ...
        if (filename_size > 0) {
            node->filename = (char*)malloc(sizeof(char) * (filename_size + 1));
        ...
        node->has_partial_data_file = 0;
        node->leaf_size = 0;
        node->split_data = (isax_node_split_data*)malloc(sizeof(isax_node_split_data));
        node->split_data->split_mask = (sax_type*)malloc(sizeof(sax_type) * index->settings->paa_segments);
...
void create_dirs(char *filename, isax_index *index) {
    struct stat st = { 0 };
    if (stat(filename, &st) == -1) {
        char *buffer;
        char *fileSep = "/";
        char *consumed = (char*) malloc(sizeof(char) * (strlen(filename) + 1));
        strcpy(consumed, filename);
        char *dir_exists = (char*) malloc(
                sizeof(char) * (strlen(index->settings->root_directory)));
        int stopPos = strrchr(index->settings->root_directory, '/') - index->settings->root_directory;
        // first round, precheck
        buffer = strtok(consumed, fileSep);
        strcpy(dir_exists, fileSep);
        strcat(dir_exists, buffer);
        // if directory does not exist, create ist
        if (stat(dir_exists, &st) == -1) {
            mkdir(dir_exists, 0777);
        }
        // now treat the entire path
        while (buffer != NULL) {
            buffer = strtok(NULL, "/");
            if (buffer != NULL) {
                strcat(dir_exists, fileSep);
                strcat(dir_exists, buffer);
                // if directory does not exist, create it
                if (stat(dir_exists, &st) == -1) {
                    mkdir(dir_exists, 0777);
                }
                // if last directory in path reached, exit loop
                if (strlen(dir_exists) >= stopPos) {
                    break;
                }
            }
        }
...
void index_write(isax_index *index) {
    ...
    char *filename = (char*)malloc(sizeof(char) * (strlen(index->settings->root_directory) + 15));
    filename = strcpy(filename, index->settings->root_directory);
    filename = strcat(filename, "/index.idx");

    create_dirs(filename, index);
    ...
    char *filename_adpt = (char*)malloc(sizeof(char) * (strlen(index->settings->root_directory) + 15));
    ...
    free(filename_adpt);
    fwrite(&(index->total_records), sizeof(uint64_t), 1, file_adpt);
    fwrite(&(index->loaded_records), sizeof(uint64_t), 1, file_adpt);
    fclose(file_adpt);
}

void index_mRecBuf_write(isax_index *index) {
    fprintf(stderr, ">>> Storing index: %s\n", index->settings->root_directory);
    char *filename = (char*)malloc(sizeof(char) * (strlen(index->settings->root_directory) + 11));
...
isax_index * index_read(const char* root_directory) {
    ...
    char *filename = (char*)malloc(sizeof(char) * (strlen(root_directory) + 15));
    ...
    raw_filename = (char*)malloc(sizeof(char) * (raw_filename_size+1));
    ...
    idx_settings->raw_filename = (char*)malloc(sizeof(char) * 256);
    ...
    if (file_adpt) {
        fread(&index->total_records, sizeof(uint64_t), 1, file_adpt);
        fread(&index->loaded_records, sizeof(uint64_t), 1, file_adpt);
        fclose(file_adpt);
    }
...
void get_index_size(isax_index *index, struct stat *stat_index, struct stat *stat_adaptive) {
    char *filename_index = (char*)malloc(sizeof(char) * (strlen(index->settings->root_directory) + 15));
...
void create_wedges(isax_index *index, isax_node *node) {
    if (node == NULL) {
        ...
        if (node->is_leaf) {
            node->wedges = (ts_type*)malloc(sizeof(ts_type) * index->settings->timeseries_size * 2);
            int i;
            ...
            if (node->has_partial_data_file) {
                ...
                ts_type *tmp_ts = (ts_type*)malloc(sizeof(ts_type) * index->settings->timeseries_size);

                filename = (char*)malloc((strlen(node->filename) + 10) * sizeof(char));
                ...
                uint32_t size = ftello(leaf_file) / index->settings->partial_record_size;
                fseeko(leaf_file, 0L, SEEK_SET);

                positions = (file_position_type*)malloc(index->settings->position_byte_size * size);
            ...
            if (node->has_full_data_file) {
                ...
                fseeko(leaf_file, 0L, SEEK_END);
                uint32_t size = ftello(leaf_file) / index->settings->full_record_size;
                fseeko(leaf_file, 0L, SEEK_SET);
...
meminfo get_memory_utilization_info(isax_index *index) {
    ...
    memcpy(&bytes_mem_info, &(index->memory_info), sizeof(meminfo));
