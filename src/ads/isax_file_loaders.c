...
void isax_query_binary_file(const char *ifilename, int q_num, isax_index *index,
    ...
    if (index->settings->function_type == 4) {
        int32_t ts_length = index->settings->timeseries_size;
    ...
    while (q_loaded < q_num) {
        ...
        } else {  // Parse ts and make PAA representation
            paa_from_ts(ts, paa,
...
void isax_query_binary_file_traditional(
    ...
    if (filetype_int) {
        ts_int32 = (file_type*)malloc(sizeof(file_type) * index->settings->timeseries_size);
    }
...
void isax_knn_query_binary_file(const char *ifilename, const char *labelfilename, int q_num, isax_index *index,
    ...
    datalabel = (int32_t*)malloc(sizeof(int32_t) * index->sax_cache_size);
    fread(datalabel, sizeof(int32_t), index->sax_cache_size, lfile);
    ...
    while (q_loaded < q_num) {
        ...
        COUNT_OUTPUT2_TIME_END
        COUNT_TOTAL_TIME_END
        int32_t *classcounter = (int32_t*)malloc(sizeof(int32_t) * classlength);
        int32_t classeposition = 0;
        int32_t classtmp = 0;
...
void isax_knn_query_binary_file_traditional(const char *ifilename, const char *labelfilename, int q_num, isax_index *index,
    ...
    datalabel = (uint32_t*)malloc(sizeof(uint32_t) * index->sax_cache_size);
    fread(datalabel, sizeof(uint32_t), index->sax_cache_size, lfile);
...
void isax_query_binary_file_batch(const char *ifilename, int q_num, isax_index *index,
    ...
    int q_loaded = 0;
    ts_type *ts = (ts_type*)malloc(sizeof(ts_type) * index->settings->timeseries_size * q_num);
    ts_type *paa = (ts_type*)malloc(sizeof(ts_type) * index->settings->paa_segments * q_num);
    ...
    while (ts_loaded < ts_num) {
        ...
        *pos = ftell(ifile);
        ...
        if (sax_from_ts(ts, sax, index->settings->ts_values_per_paa_segment,
            ...
            snprintf(pfilename, sizeof(pfilename), "%s.%llu", index->settings->raw_filename, first_bit_mask);
...
void isax_sorted_index_binary_file(const char *ifilename, int ts_num, isax_index *index) {
    ...
    ts_type *ts = (ts_type*)malloc(sizeof(ts_type) * index->settings->timeseries_size);
    sax_vector *sax_vectors = (sax_vector*)malloc(sizeof(sax_vector) * ts_num);
...
void isax_merge_sorted_index_binary_file(const char *ifilename, int ts_num, isax_index *index) {
    ...
    uint32_t i;
    int segment;
    sax_type *removals = (sax_type*)malloc(sizeof(sax_type) * index->settings->paa_segments);
    ...
    sax_vector *new_dataset = (sax_vector*)malloc(sizeof(sax_vector) * ts_num);
