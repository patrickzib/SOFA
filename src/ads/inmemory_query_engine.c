...
#define NTHREADS 4
#define SET_TO_SMALLER(dst, src) src < dst ? src : dst
int checkts = 0;
float *MINDISTS;
...
void *compute_mindists_in(void *ptr) {
    ...
    for (count = arguments->from; count < arguments->to; count++) {
        sax_type *sax = &(arguments->index->sax_cache[count * arguments->index->settings->paa_segments]);
#if defined(__x86_64__)
        MINDISTS[count] = minidist_paa_to_isax_rawa_SIMD(arguments->paa, sax,
                arguments->index->settings->max_sax_cardinalities, arguments->index->settings->sax_bit_cardinality,
                arguments->index->settings->sax_alphabet_cardinality, arguments->index->settings->paa_segments,
                MINVAL, MAXVAL, arguments->index->settings->mindist_sqrt);
#else
        MINDISTS[count] = minidist_paa_to_isax_raw(arguments->paa, sax,
                arguments->index->settings->max_sax_cardinalities, arguments->index->settings->sax_bit_cardinality,
                arguments->index->settings->sax_alphabet_cardinality, arguments->index->settings->paa_segments,
                MINVAL, MAXVAL, arguments->index->settings->mindist_sqrt);
#endif
...
query_result approximate_search_inmemory(ts_type *ts, ts_type *paa, isax_index *index) {
    ...
    sax_from_paa(paa, sax, index->settings->paa_segments,  index->settings->sax_alphabet_cardinality,
...
float calculate_node_distance_inmemory(isax_index *index, isax_node *node, ts_type *query, float bsf) {
    ...
    if (node->buffer != NULL) {
        ...
        for (count = 0; count < node->buffer->full_buffer_size; count++) {
            float dist = ts_ed(query, node->buffer->full_ts_buffer[count],
                    index->settings->timeseries_size, bsf, index->settings->SIMD_flag, index->settings->is_norm);
            bsf = SET_TO_SMALLER(bsf, dist);
        }

        RDcalculationnumber = RDcalculationnumber + node->buffer->tmp_full_buffer_size;

        for (count = 0; count < node->buffer->tmp_full_buffer_size; count++) {
            float dist = ts_ed(query, node->buffer->tmp_full_ts_buffer[count],
                    index->settings->timeseries_size, bsf, index->settings->SIMD_flag, index->settings->is_norm);
            bsf = SET_TO_SMALLER(bsf, dist);
        }
        ...
        for (count = 0; count < node->buffer->partial_buffer_size; count++) {
            float dist = ts_ed(query, &(rawfile[*node->buffer->partial_position_buffer[count]]),
                    index->settings->timeseries_size, bsf, index->settings->SIMD_flag, index->settings->is_norm);
            bsf = SET_TO_SMALLER(bsf, dist);
        }
    }
...
ts_type *calculate_node_ts_distance_inmemory(isax_index *index, isax_node *node, ts_type *query, float bsf) {
    ...
    if (node->buffer != NULL) {
        ...
        for (count = 0; count < node->buffer->full_buffer_size; count++) {
            ...
            if (dist <= bsf) {
                bsf = dist;
                result = node->buffer->full_ts_buffer[count];
            }
        }
        ...
        for (count = 0; count < node->buffer->tmp_full_buffer_size; count++) {
            ...
            if (dist <= bsf) {
                bsf = dist;
                result = node->buffer->tmp_full_ts_buffer[count];
            }
        }
        ...
        for (count = 0; count < node->buffer->partial_buffer_size; count++) {
            ...
            if (dist <= bsf) {
                bsf = dist;
                result = &(rawfile[*node->buffer->partial_position_buffer[count]]);
            }
        }
    }
...
float calculate_node_distance2_inmemory(isax_index *index, isax_node *node, ts_type *query, ts_type *paa, float bsf) {
    ...
    if (node->buffer != NULL) {
#if defined(__x86_64__)
        if (index->settings->SIMD_flag && __builtin_cpu_supports("avx512dq")) {
            for (int count = 0; count < node->buffer->partial_buffer_size; count++) {
                distmin = minidist_paa_to_isax_rawa_SIMD512(paa, node->buffer->partial_sax_buffer[count],
                        index->settings->max_sax_cardinalities, index->settings->sax_bit_cardinality,
                        index->settings->sax_alphabet_cardinality, index->settings->paa_segments,
                        MINVAL, MAXVAL, index->settings->mindist_sqrt);

                if (distmin < bsf) {
                    float dist = ts_euclidean_distance_SIMD512DQ(query, &(rawfile[*node->buffer->partial_position_buffer[count]]),
            ...
            for (int count = 0; count < node->buffer->partial_buffer_size; count++) {
                distmin = minidist_paa_to_isax_rawa_SIMD(paa, node->buffer->partial_sax_buffer[count],
                        index->settings->max_sax_cardinalities, index->settings->sax_bit_cardinality,
                        index->settings->sax_alphabet_cardinality, index->settings->paa_segments,
                        MINVAL, MAXVAL, index->settings->mindist_sqrt);

                if (distmin < bsf) {
                    float dist = ts_euclidean_distance_SIMD(query, &(rawfile[*node->buffer->partial_position_buffer[count]]),
                            index->settings->timeseries_size, bsf);
                    bsf = SET_TO_SMALLER(bsf, dist);
                }
            }
...
#endif
            for (int count = 0; count < node->buffer->partial_buffer_size; count++) {
                distmin = minidist_paa_to_isax_raw(paa, node->buffer->partial_sax_buffer[count],
                        index->settings->max_sax_cardinalities, index->settings->sax_bit_cardinality,
                        index->settings->sax_alphabet_cardinality, index->settings->paa_segments,
                        MINVAL,  MAXVAL, index->settings->mindist_sqrt);

                if (distmin < bsf) {
                    float dist = ts_euclidean_distance(query, &(rawfile[*node->buffer->partial_position_buffer[count]]),
                            index->settings->timeseries_size, bsf);
                    bsf = SET_TO_SMALLER(bsf, dist);
                }
            }
...
float calculate_node_distance2_inmemory_SFA(isax_index *index, isax_node *node, ts_type *query, ts_type *query_fft,
    ...
    if (node->buffer != NULL) {
        for (int count = 0; count < node->buffer->partial_buffer_size; count++) {
            distmin = minidist_fft_to_sfa_raw(index, query_fft, node->buffer->partial_sax_buffer[count],
                    index->settings->max_sax_cardinalities, bsf);

            if (distmin < bsf) {
                float dist;
                dist = ts_euclidean_distance(query, &(rawfile[*node->buffer->partial_position_buffer[count]]),
                        index->settings->timeseries_size, bsf);
                bsf = SET_TO_SMALLER(bsf, dist);
            }
        }
...
#if defined(__x86_64__)
float calculate_node_distance2_inmemory_SFA_SIMD(isax_index *index, isax_node *node, ts_type *query, ts_type *query_fft,
        float bsf) {
    COUNT_CHECKED_NODE()
    float distmin;
    // If node has buffered data
    ...
    if (node->buffer != NULL) {
        if (__builtin_cpu_supports("avx512vl")) {
            for (int count = 0; count < node->buffer->partial_buffer_size; count++) {
                distmin = minidist_fft_to_sfa_rawe_SIMD512(index, query_fft, node->buffer->partial_sax_buffer[count],
                        index->settings->max_sax_cardinalities, bsf);

                if (distmin < bsf) {
                    float dist;
                    dist = ts_euclidean_distance_SIMD512DQ(query, &(rawfile[*node->buffer->partial_position_buffer[count]]),
                            index->settings->timeseries_size, bsf);
                    bsf = SET_TO_SMALLER(bsf, dist);
                }
            }
            ...
            for (int count = 0; count < node->buffer->partial_buffer_size; count++) {
                distmin = minidist_fft_to_sfa_rawe_SIMD(index, query_fft, node->buffer->partial_sax_buffer[count],
                        index->settings->max_sax_cardinalities, bsf);

                if (distmin < bsf) {
                    float dist;
                    dist = ts_euclidean_distance_SIMD(query, &(rawfile[*node->buffer->partial_position_buffer[count]]),
                            index->settings->timeseries_size, bsf);
                    bsf = SET_TO_SMALLER(bsf, dist);
                }
            }
...
void calculate_node_topk_inmemory_SFA(isax_index *index, isax_node *node, ts_type *query, ts_type *query_fft,
            ...
            if (distmin < pq_bsf->knn[pq_bsf->k - 1]) {
                ...
                if (dist <= pq_bsf->knn[pq_bsf->k - 1]) {
                    ...
                    pqueue_bsf_insert(pq_bsf, dist,
                            *node->buffer->partial_position_buffer[count] / index->settings->timeseries_size, node);
...
#if defined(__x86_64__)
void calculate_node_topk_inmemory_SFA_SIMD(isax_index *index, isax_node *node, ts_type *query, ts_type *query_fft,
        pqueue_bsf *pq_bsf, pthread_rwlock_t *lock_queue) {
    COUNT_CHECKED_NODE()
    float distmin;
...
query_result exact_search_serial_inmemory(ts_type *ts, ts_type *paa, isax_index *index, float minimum_distance,
    ...
    // FOR THREAD USE
    MINDISTS = (float*)malloc(sizeof(float) * index->sax_cache_size);
    ...
    for (count = 0; count < 1; count++) {
        arguments[count].i = count;
        arguments[count].from = count * (index->sax_cache_size / 1);
        arguments[count].bsf = approximate_result.distance;

        if (count < (1 - 1)) {
            arguments[count].to = (count + 1) * (index->sax_cache_size / 1);

        } else {
            arguments[count].to = index->sax_cache_size;
        }

        arguments[count].paa = paa;
        arguments[count].index = index;
        int ret = pthread_create(&thread[count], NULL, compute_mindists_in, &arguments[count]);
    }
    ...
    for (count = 0; count < index->sax_cache_size; count++) {
        sax_type *sax = &index->sax_cache[count * index->settings->paa_segments];

        if (MINDISTS[count] <= approximate_result.distance) {
            ts_buffer = &rawfile[count * index->settings->timeseries_size];
            checkts++;
#if defined(__x86_64__)
            float dist = __builtin_cpu_supports("avx512dq") ?
                    ts_euclidean_distance_SIMD512DQ(ts, ts_buffer, index->settings->timeseries_size, FLT_MAX) :
                    ts_euclidean_distance_SIMD(ts, ts_buffer, index->settings->timeseries_size, FLT_MAX);
#else
            float dist = ts_euclidean_distance(ts, ts_buffer, index->settings->timeseries_size, FLT_MAX);
#endif
...
query_result exact_search_serial_1bsf_inmemory(ts_type *ts, ts_type *paa, isax_index *index, float minimum_distance,
    ...
    // FOR THREAD USE
    MINDISTS = (float*)malloc(sizeof(float) * index->sax_cache_size);
    unsigned long diskconter = 0;
    unsigned long bsfupdate = 0;
        ...
        if (node->buffer != NULL) {
            for (i = 0; i < node->buffer->partial_buffer_size; i++) {
                ...
                float mindist = minidist_paa_to_isax_raw_SIMD(query, node->buffer->partial_sax_buffer[i],
                        ...
                        index->settings->mindist_sqrt);
#else
                float mindist = minidist_paa_to_isax_raw(query, node->buffer->partial_sax_buffer[i],
                        index->settings->max_sax_cardinalities,
                        index->settings->sax_bit_cardinality,
                        index->settings->sax_alphabet_cardinality,
                        index->settings->paa_segments, MINVAL, MAXVAL,
            ...
            for (i = 0; i < node->buffer->tmp_partial_buffer_size; i++) {
                ...
                float mindist = minidist_paa_to_isax_raw_SIMD(query, node->buffer->tmp_partial_sax_buffer[i],
                        ...
                        index->settings->mindist_sqrt);
#else
                float mindist = minidist_paa_to_isax_raw(query, node->buffer->tmp_partial_sax_buffer[i],
                        index->settings->max_sax_cardinalities,
                        index->settings->sax_bit_cardinality,
                        index->settings->sax_alphabet_cardinality,
                        index->settings->paa_segments, MINVAL, MAXVAL,
...
#if defined(__x86_64__)
    if (index->settings->SIMD_flag && __builtin_cpu_supports("avx512dq")) {
        distance = minidist_paa_to_isax_rawa_SIMD512(paa, node->isax_values, node->isax_cardinalities,
        ...
        distance =  minidist_paa_to_isax_rawa_SIMD(paa, node->isax_values, node->isax_cardinalities,
                index->settings->sax_bit_cardinality, index->settings->sax_alphabet_cardinality,
                index->settings->paa_segments, MINVAL, MAXVAL, index->settings->mindist_sqrt);
    } else {
#endif
        distance = minidist_paa_to_isax(paa, node->isax_values, node->isax_cardinalities,
                index->settings->sax_bit_cardinality, index->settings->sax_alphabet_cardinality,
                index->settings->paa_segments, MINVAL, MAXVAL, index->settings->mindist_sqrt);
#if defined(__x86_64__)
    }
