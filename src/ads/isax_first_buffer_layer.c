...
struct first_buffer_layer * initialize_fbl(int initial_buffer_size, int number_of_buffers,
    ...
    struct first_buffer_layer *fbl = (first_buffer_layer*)malloc(sizeof(first_buffer_layer));
    ...
    int64_t hard_buffer_size = (int64_t)(index->settings->sax_byte_size + index->settings->position_byte_size) * (int64_t) max_total_buffers_size;
    ...
    for (int count = 0; count < number_of_buffers; count++) {
        fbl->soft_buffers[count].initialized = 0;
        fbl->soft_buffers[count].max_buffer_size = 0;
        fbl->soft_buffers[count].buffer_size = 0;
    }
...
struct parallel_first_buffer_layer * initialize_pRecBuf(int initial_buffer_size, int number_of_buffers,
        int max_total_buffers_size, isax_index *index) {
    struct parallel_first_buffer_layer *fbl = (parallel_first_buffer_layer*)malloc(sizeof(parallel_first_buffer_layer));
    ...
    for (int count = 0; count < number_of_buffers; count++) {
        fbl->soft_buffers[count].initialized = 0;
        fbl->soft_buffers[count].finished = 0;
    }
...
struct first_buffer_layer2 * initialize_simrec(int initial_buffer_size, int number_of_buffers,
        int max_total_buffers_size, isax_index *index) {
    struct first_buffer_layer2 *fbl = (first_buffer_layer2*)malloc(sizeof(first_buffer_layer2));
    ...
    // Allocate a set of soft buffers to hold pointers to the hard buffer
    fbl->soft_buffers = (fbl_soft_buffer2*)malloc(sizeof(fbl_soft_buffer2) * number_of_buffers);
    fbl->current_record_index = 0;
...
struct parallel_dfirst_buffer_layer * initialize_2pRecBuf(int initial_buffer_size, int number_of_buffers,
        int max_total_buffers_size, isax_index *index) {
    struct parallel_dfirst_buffer_layer *fbl = (parallel_dfirst_buffer_layer*)malloc(sizeof(parallel_dfirst_buffer_layer));
    ...
    // Allocate a set of soft buffers to hold pointers to the hard buffer
    fbl->soft_buffers = (parallel_dfbl_soft_buffer*)malloc(sizeof(parallel_dfbl_soft_buffer) * number_of_buffers);
    fbl->current_record_index = 0;
...
enum response flush_fbl(first_buffer_layer *fbl, isax_index *index) {
    ...
    for (j = 0; j < fbl->number_of_buffers; j++) {
        ...
        if (current_fbl_node->buffer_size > 0) {
            ...
            for (int count = 0; count < current_fbl_node->buffer_size; count++) {
                r->sax = current_fbl_node->sax_records[count];
                r->position = current_fbl_node->pos_records[count];
            ...
            isax_index_clear_node_buffers(index, current_fbl_node->node,
                    INCLUDE_CHILDREN, TMP_AND_TS_CLEAN);
