...
[[ $TRIE_LEAF_IVF_LOCAL_VQ_BOUND == false || $INDEX_TYPE == trie ]] || \
    die '--trie-leaf-ivf-local-vq-bound requires --index-type trie'
...
run_one() {
    ...
    if [[ $INDEX_TYPE == trie ]]; then
        ...
        [[ $TRIE_LEAF_IVF_LOCAL_VQ_BOUND == true ]] && command+=(--trie-leaf-ivf-local-vq-bound)
